...
namespace Sitecore.Foundation.Indexing.Factories
{
    [Service]
    public class SearchResultsFactory
    {
        ...
        private bool IsFacetValueSelected(FacetCategory resultCategory, IQuery query, FacetValue resultValue)
        {
            ...
            return query.Facets.Any(f => 
                f.Key.Equals(resultCategory.Name, StringComparison.InvariantCultureIgnoreCase) && 
                f.Value.Any(v => v.Equals(resultValue.Name, StringComparison.InvariantCultureIgnoreCase))
                );
        }
        ...
        {
            return IndexingProviderRepository.QueryFacetProviders
                .SelectMany(provider => provider.GetFacets())
                .Distinct(new GenericEqualityComparer<IQueryFacet>((facet, queryFacet) => facet.FieldName == queryFacet.FieldName, facet => facet.FieldName.GetHashCode()))
                .ToDictionary(facet => facet.FieldName, facet => facet);
        }
    }
}